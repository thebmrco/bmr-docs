...
The MOS score now factors in **Bass Ratio** as a penalty on top of the **Reverberation Time** base. Earlier releases displayed Bass Ratio alongside the other measurements but did not feed it into the score. **Speech-to-Noise Ratio** is captured today and will start contributing in the next release; **Spatial Concentration** and **Flutter Echo** follow after that. This page tracks the current state.
:::
...
In release 1.2, only two terms actively contribute to the score: the **Reverberation Time** base and the **Bass Ratio** penalty. The other terms — **Speech-to-Noise Ratio**, **Spatial Concentration**, and **Flutter Echo** — are measured or computed today and shown alongside the score, but do not yet feed into it. **Speech-to-Noise Ratio** is the next one planned for inclusion; **Spatial Concentration** and **Flutter Echo** follow after that.
...
:::note Coming in the next release
SpNR is captured today and shown alongside the MOS score, but does **not yet contribute** to the score itself — that changes in the next release. The rest of this section describes how SpNR will affect the MOS score once it is folded into the formula.
:::

When the gap between speech and noise is wide, listeners pick out words effortlessly. When it shrinks, they have to lean in, ask for repeats, or zone out. The MOS score will reflect that effort directly: the noisier the room *relative to speech*, the lower the score.
...
The speech reference level used in the formula assumes a normal conversational level at typical meeting-room distances. Both values are combined into a single SpNR figure that will feed the MOS score once SpNR is part of the formula.

:::caution Stay quiet after the 3-2-1
Because the SpNR window is short, it is **very important to avoid any noise during the measurement** — once the **3-2-1 countdown** finishes, the room should be as quiet as possible. Talking, chair movement, or a door opening in that window will pull the SpNR figure down, and once SpNR is folded into the formula it will lower the MOS score for the measurement.
:::
...
<img
  src={useBaseUrl('/img/release/2026-07-ambient-noise.png')}
  alt="Ambient Noise chart with Ideal, Acceptable, and Too loud acceptability zones and three measurements plotted across frequencies"
...
<img
  src={useBaseUrl('/img/release/2026-07-rt60-chart.png')}
  alt="T20 — Reverberation Time (RT60) chart with the 125 Hz band shaded and labelled Invalid measurement while the valid bands form a clean curve inside the Green Zone"
  style={{width: '100%', maxWidth: '560px', display: 'block', margin: '1rem auto', borderRadius: '8px', border: '1px solid #E5E5D5'}}
/>
...
:::info Coming soon
**Speech-to-Noise Ratio** is the next acoustic property planned for inclusion in the MOS score — it is measured today but does not yet contribute to the score. **Spatial Concentration** and **Flutter Echo** follow after that.
