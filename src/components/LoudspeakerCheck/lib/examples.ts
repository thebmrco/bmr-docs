...
export const EXAMPLES: Example[] = [
  ...
  },
  {
    ...
    speaker: {
      ...
      power_mode: 'battery',
      woofer_mm: 70,
      sensitivity_db_1w_1m: 80,
      sensitivity_estimated: true,
      connection: 'bluetooth',
      bluetooth_codec: 'Bluetooth 5.3, A2DP 1.3',
      note: 'Official spec sheet: 2 × 70 mm woofers, 2 × 20 mm tweeters, 44 Hz – 20 kHz. Rated output 2 × 20 W + 2 × 15 W RMS on battery (70 W) and 2 × 30 W + 2 × 20 W on AC (100 W) — the battery figure is used here, since a room measurement runs unplugged. No maximum SPL is published. USB-C only; there is no analogue input.',
    },
    why: 'The loudspeaker used in the reduced measurement chain of the MEng thesis, against a Genelec 8020 and an NTi measurement microphone in three rooms. Its results in the 250 Hz – 2 kHz bands tracked the professional reference closely enough to make room decisions from. Note that its DSP applies programme-dependent gain at high level, which is time-variant and is not separated out by the sweep.',
    sourceUrl:
  ...
  },
  {
    ...
    speaker: {
      ...
      power_mode: 'battery',
      woofer_mm: 45,
      sensitivity_db_1w_1m: 78,
      sensitivity_estimated: true,
      connection: 'bluetooth',
      bluetooth_codec: 'SBC',
      note: 'The cheap end of the range, about €60. 7 W, one 45 mm driver, 95 Hz – 20 kHz.',
    },
    why: 'A case where level is not the limit. Its stated 95 Hz sits inside the 125 Hz octave (88.4–177 Hz), so that band is played, but weaker, and a 45 mm driver runs out of excursion in the bands below 250 Hz first: the 125 Hz band may drop out.',
  ...
  },
  {
    ...
    why: 'A class average for portable Bluetooth loudspeakers in the €60–150 range, built from what the manufacturer publishes — and JBL publishes no maximum SPL for any of them, so the level here is derived from an assumed 78 dB/W/m driver sensitivity. That assumption is worth ±4 dB across this driver class, and independent reviewers measuring the Flip 6 report figures more than 10 dB apart, which is exactly why this result carries the lowest confidence of the set.',
